...
export default async function LaunchpadPage({
  ...
  const allDetailsExist =
    agency.address && agency.agencyLogo && agency.city &&
    agency.companyEmail && agency.companyPhone && agency.country &&
    agency.name && agency.state && agency.zipCode;

  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="text-xl font-semibold tracking-tight sm:text-2xl">Launchpad</h1>
        <p className="mt-1 text-[13px] text-muted-foreground">Follow the steps below to get your account set up.</p>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-4 rounded-lg border p-4">
            ...
            {agency.connectAccountId ? (
              ...
              <ConnectStripeButton agencyId={agencyId} />
            )}
          </div>

          <div className="flex items-center justify-between gap-4 rounded-lg border p-4">
            <div className="flex items-center gap-4">
              <Image src={agency.agencyLogo} alt="Agency" height={48} width={48} className="rounded-md object-contain" />
