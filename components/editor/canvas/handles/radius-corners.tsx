...
export function RadiusCorners({ element, h }: { element: El; h: ReturnType<typeof useHandles> }) {
  ...
  return <>{CORNERS.map(({ id, prop, pos, cursor }) => {
    ...
    return (
      <div
        key={id}
        className={cn('absolute z-20 flex items-center justify-center size-5', cursor, isActive || isHovered ? 'opacity-100' : 'opacity-0 hover:opacity-100', 'transition-opacity', pos)}
        onPointerDown={(e) => h.dragRadius(element, id, prop, e)}
        onPointerEnter={() => h.hover(id)}
        onPointerLeave={() => h.hover(null)}
      >
        <div className={cn('rounded-full transition-transform', isActive ? 'bg-orange-500 size-2.5 scale-110' : isHovered ? 'bg-primary size-2' : r > 0 ? 'bg-primary/60 size-1.5' : 'bg-primary/30 size-1.5')} />
        {(isActive || isHovered) && <span className={cn('absolute rounded px-1 py-px text-[8px] font-mono text-white whitespace-nowrap pointer-events-none shadow z-30', isActive ? 'bg-orange-500' : 'bg-primary/80', id.includes('TL') ? '-top-4 left-0' : id.includes('TR') ? '-top-4 right-0' : id.includes('BR') ? '-bottom-4 right-0' : '-bottom-4 left-0')}>{r}</span>}
      </div>
    );
  })}</>;
}
