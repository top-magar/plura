 ...
 */
export default function Guides({ zoom, scrollLeft, scrollTop, canvasOffsetX, canvasOffsetY }: { zoom: number; scrollLeft: number; scrollTop: number; canvasOffsetX: number; canvasOffsetY: number }): ReactNode {
  ...
  return (
    <>
      ...
      {guides.map((g, i) => {
        const pos = g.axis === 'x'
          ? { left: g.position * z + canvasOffsetX - scrollLeft, top: 0, width: 1, height: '100%' as const, cursor: 'col-resize' as const }
          : { top: g.position * z + canvasOffsetY - scrollTop, left: 0, height: 1, width: '100%' as const, cursor: 'row-resize' as const };
        return (
          <div
            key={`${g.axis}-${i}`}
      ...
      {dragging && (
        <div className="absolute z-50 pointer-events-none" style={dragging.axis === 'x'
          ? { left: dragging.pos * z + canvasOffsetX - scrollLeft, top: 0, width: 1, height: '100%', backgroundColor: '#818cf8' }
          : { top: dragging.pos * z + canvasOffsetY - scrollTop, left: 0, height: 1, width: '100%', backgroundColor: '#818cf8' }
        }>
