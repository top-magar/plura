...
function editorReducer(state: EditorState, action: EditorAction): EditorState {
  switch (action.type) {
    case "ADD_ELEMENT": {
      ...
      const newEditor = { ...state.editor, elements: newElements };
      const newHistory = [...state.history.history.slice(0, state.history.currentIndex + 1), { ...newEditor }];
      return { editor: newEditor, history: { history: newHistory, currentIndex: newHistory.length - 1 } };
    }
    ...
    case "DELETE_ELEMENT": {
      ...
      const newEditor = { ...state.editor, elements: newElements, selectedElement: null };
      const newHistory = [...state.history.history.slice(0, state.history.currentIndex + 1), { ...newEditor }];
      return { editor: newEditor, history: { history: newHistory, currentIndex: newHistory.length - 1 } };
    }

    case "SELECT_ELEMENT":
      return { ...state, editor: { ...state.editor, selectedElement: action.payload.element } };
    ...
    case "UNDO": {
      ...
      return { editor: { ...prevEditor, selectedElement: null }, history: { ...state.history, currentIndex: prevIndex } };
    }
    ...
    case "REDO": {
      ...
      return { editor: { ...nextEditor, selectedElement: null }, history: { ...state.history, currentIndex: nextIndex } };
    }

    case "LOAD_DATA": {
      const newEditor = { ...state.editor, elements: action.payload.elements, funnelPageId: action.payload.funnelPageId };
      return {
        editor: newEditor,
...
const EditorContext = createContext<{ state: EditorState; dispatch: Dispatch<EditorAction> }>({
